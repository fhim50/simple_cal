...
class Calculator:
	...
	def press_key(self, key):
		self.equal_press = self.change_state(self.equal_press)
		content = console.get()
		key = str(key)
		if self.key_press:
			self.pressed_key = key
			self.key_press = self.change_state(self.key_press)
	...
	def compute(self):
		...
		# self.results = float(console.get())
		# return self.estimate() if self.pending_operation else self.results
	...
	def estimate(self):
		...
		self.key_press = self.change_state(self.key_press)
		self.pending_operation = self.change_state(self.pending_operation)
	...
	def operation(self, operator):
		...
		elif self.change_state(self.equal_press):
